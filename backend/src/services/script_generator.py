...
class ScriptGenerationService:
    ...
    def _build_style_instruction(self) -> str:
        ...
        style_instructions = {
            "plain": (
                "采用“通俗解释”节目风格。\n"
                "- 目标听感：像两个懂行的朋友把复杂问题讲明白，适合通勤、运动时轻松收听。\n"
                "- 内容策略：先给结论，再用生活化类比解释概念；术语必须跟一句白话解释。\n"
                "- 对话分工：Host 多问“这和普通人有什么关系”“能不能打个比方”；Guest 用案例、类比和简短判断回答。\n"
                "- 信息密度：中等，避免连续堆数据；每轮只推进一个核心点。\n"
                "- 情绪标注：亲和、好奇、恍然大悟为主，不要过度兴奋或大幅提高音量。"
            ),
            "professional": (
                "采用“专业分析”节目风格。\n"
                "- 目标听感：像一场节奏清晰的行业分析对谈，信息密度高但不念报告。\n"
                "- 内容策略：围绕“背景-机制-影响-风险-判断”推进，保留关键概念、因果链和边界条件。\n"
                "- 对话分工：Host 负责追问假设、反例、影响范围；Guest 给出结构化判断、证据和谨慎表述。\n"
                "- 信息密度：偏高，可以出现数据和专业名词，但必须解释其意义。\n"
                "- 情绪标注：克制、笃定、分析式表达为主，少用夸张口头禅和强情绪词。"
            ),
            "news": (
                "采用“新闻播报”节目风格。\n"
                "- 目标听感：像新闻专题里的双人快评，事实优先、节奏紧凑、少闲聊。\n"
                "- 内容策略：开场直接交代发生了什么、为什么重要、影响谁；后续按进展、各方反应、风险和后续观察推进。\n"
                "- 对话分工：Host 做导语、转场和关键信息提示；Guest 做简明解读和影响判断。\n"
                "- 信息密度：偏高，句子更短，减少比喻和玩笑，不使用冗长铺垫。\n"
                "- 情绪标注：沉稳、清晰、节奏略快为主，不要像娱乐闲聊，也不要大幅表演化。"
            ),
        }
